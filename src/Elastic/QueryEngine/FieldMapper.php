...
class FieldMapper {
	 ...
	 * @return string
	 */
	public function getFieldType( DIProperty $property ) {
	 ...
	 * @return string
	 */
	public function getField( DIProperty $property, $affix = '' ) {
		return $this->getFieldType( $property ) . $affix;
	}
	 ...
	 * @return []
	 */
	public function elastic_result( array $results, $cutoff = null ) {
		...
		// https://www.elastic.co/guide/en/elasticsearch/client/php-api/6.0/_search_operations.html
	...
	//	$info['total'] = $results['hits']['max_score'];

		return [ $res, $info, $scores, $continue ];
	}
	...
	public function match( $field, $value, $operator = 'or' ) {
		...
		if ( is_array( $field ) ) {
			return $this->multi_match( $field, $value, [ "type" => "phrase" ] );
		}

		// Is it a phrase match as in "Foo bar"?
		if ( $value !=='' && $value{0} === '"' && substr( $value, -1 ) === '"' ) {
			return $this->match_phrase( $field, substr( $value, 1, -1 ) );
		}
	...
	public function wildcard( $field, $value ) {
		return [ 'wildcard' => [ "$field" => mb_strtolower( $value ) ] ];
	}
